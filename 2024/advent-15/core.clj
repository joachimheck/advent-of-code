  ...
  (let [move-fn (make-move-fn move)]
    (if (some #{\<\>} [move])
      (let [stuff-ahead (take-while #(not (some #{(second %)} [\#\.])) (map #(vector % (get grid %)) (rest (iterate move-fn robot-pos))))
            ;; _ (if (nil? (first stuff-ahead))
            ;;     (do (println "stuff-ahead" stuff-ahead ", move:" move "[width height]:" [(:width grid) (:height grid)])
            ;;         (print-grid {:grid grid :width 20 :height 10 :robot-pos robot-pos})))
            next (if (empty? stuff-ahead)
                   (get grid (move-fn robot-pos))
                   (get grid (move-fn (first (last stuff-ahead)))))]
        (if (= \. next)
...
(defn move-next-wide [{grid :grid width :width height :height robot-pos :robot-pos moves :moves :as state}]
  (let [move (first moves)
        ...
        [new-grid new-robot-pos] (cond
                                   ...
                                   :else
                                   (let [new-box-positions (map move-fn boxes-ahead)
                                         ;; _ (println "new-box-positions" new-box-positions)
                                         empty-spaces (remove (set new-box-positions) boxes-ahead)
                                         grid-with-boxes-removed (reduce (fn [grid [x y :as space]]
                                                                           (-> grid
                                                                               (assoc space \.)
                                                                               (assoc [(inc x) y] \.)))
                                                                         grid
                                         ...
                                         grid-with-moved-boxes (reduce (fn [grid [x y :as box]]
                                                                         (-> grid
                                                                             (assoc box \[)
                                                                             (assoc [(inc x) y] \])))
                                                                       grid-with-boxes-removed
                                                                       new-box-positions)
                                         ]
                                     [grid-with-moved-boxes (move-fn robot-pos)]))]
    (-> state
        (assoc :moves (rest moves))
        (assoc :grid new-grid)
        (assoc :robot-pos new-robot-pos))))
...
;; (time (sum-gps-coordinates-wide small-input))
;; ####################
;; ##[].......[].[][]##
;; ##[]...........[].##
;; ##[]........[][][]##
...
;; ##......[][]..[]..##
;; ####################
;; "Elapsed time: 9.9851 msecs"
;; 9021
;; (time (sum-gps-coordinates-wide large-input))
;; ####################################################################################################
;; ##......[]..........[]..[]..[]##....[]......[][]##..[][][]....[]##..........[]....[]##....[]##....##
;; ##..[]..##..[]##..................##[]....[][][]##[][]....[].[].[]##............[]....[]..........##
;; ##....[][]....[]......[][]....[]##....[][]##....[].[][].........[]............................[]..##
