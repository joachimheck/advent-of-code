...
(defn is-maybe-tree-top-lines? [{robots :robots [width height :as dimensions] :dimensions :as state}]
...
(defn is-maybe-tree-thirds? [{robots :robots [width height :as dimensions] :dimensions :as state}]
  (let [third (quot width 3)]
    (>
     (count
      (for [robot robots
            :when (<= third (first (:p robot)) (* 2 third))]
        robot))
     (/ (count robots) 2))))

(defn is-maybe-tree-ninths? [{robots :robots [width height :as dimensions] :dimensions :as state}]
  (let [x-third (quot width 3)
        y-third (quot height 3)]
    (>
     (count
      (for [robot robots
            :when (and (<= x-third (first (:p robot)) (* 2 x-third))
                       (<= y-third (second (:p robot)) (* 2 y-third)))]
        robot))
     (/ (count robots) 3))))

(defn is-maybe-tree-heavy-quadrant? [{robots :robots [width height :as dimensions] :dimensions :as state}]
  (let [x-half (quot width 2)
        y-half (quot height 2)
        by-quadrant (reduce (fn [acc robot]
                              (update acc (which-quadrant (:p robot) dimensions) inc))
                            {nil 0
                             :upper-left 0
                             :upper-right 0
                             :lower-left 0
                             :lower-right 0}
                            robots)]
    (some #(> % (/ (count robots) 2)) (vals by-quadrant))))
  ...
  (let [{robots :robots [width height :as dimensions] :dimensions :as state} (parse-input input)
    ...
    (loop [seconds 0
      ...
      (let [line-counts (:line-counts state)]
        (cond (> seconds max-seconds)
              nil
              (is-maybe-tree-heavy-quadrant? state)
...
;; Hint (/ solution?) from Reddit: the tree will be in the center so divide the area into
;; nine "quadrants" and check that most of the robots are in the middle ones.
;; Or three vertical strips, I guess.

;; That was kind of it - I used the original quadrants and it popped right out.

;; (how-long-until-christmas-tree-step large-input 10000)
;; ..................*.....................................................*..........................*.
;; .....................................................................................................
;; .....................................................................................................
;; .....................................................*..........*....*...............................
