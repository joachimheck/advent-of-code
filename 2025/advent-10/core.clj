...
(defn increment-presses [presses buttons goal]
  ...
  ;; (let [max-presses (for [b buttons] (get-max-presses b goal))
  ;;       _ (println "max-presses" max-presses)
  ;;       indexed (map-indexed list (map list presses goal))
  ;;       filtered (filter #(< (first (second %)) (second (second %))) indexed)]
