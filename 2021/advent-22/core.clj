...
(defn add-cube [[ax1 ax2 ay1 ay2 az1 az2 :as a] [bx1 bx2 by1 by2 bz1 bz2 :as b]]
  ...
  (let [result (distinct (remove nil? (concat (subtract-cube a b) (subtract-cube b a) (list (overlap a b)))))]
    (if (some (fn [[x1 x2 y1 y2 z1 z2 :as cuboid]] (or (nil? cuboid) (> x1 x2) (> y1 y2) (> z1 z2))) result)
...
(defn reboot-reactor [instructions]
  (let [cuboids (reduce (fn [cuboids {operation :operation op-cuboid :cuboid :as instruction}]
                          (if (= operation "on")
                            ;; subtract each existing cuboid from the new cuboid, then add what's left.
                            (concat cuboids
                                    (reduce (fn [op-sub-cuboids cuboid]
                                              (apply concat (map (fn [op-sub-cuboid] (subtract-cube op-sub-cuboid cuboid)) op-sub-cuboids)))
                                            (list op-cuboid)
                                            cuboids))
...
;; (time (apply + (map count-cubes (reboot-reactor (parse-input small-input-2)))))
;; "Elapsed time: 170.9503 msecs"
;; 2758514936282235

;; (time (apply + (map count-cubes (reboot-reactor (parse-input large-input)))))
;; "Elapsed time: 8122.5505 msecs"
;; 1130514303649907
